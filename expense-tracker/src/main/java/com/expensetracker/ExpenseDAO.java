...
public class ExpenseDAO {
    ...
    public static List<Expense> getExpensesByRange(LocalDate from, LocalDate to) {
        List<Expense> list = new ArrayList<>();
        String sql = """
                SELECT e.id, e.amount, e.date,
                       c.id AS cid, c.name AS cname,
                       e.project, e.payment_method, e.reimbursed,
                       e.note
                FROM expenses e
        ...
        try (Connection conn = Database.getConnection();
            ...
            try (ResultSet rs = ps.executeQuery()) {
                while (rs.next()) {
                    ...
                    boolean reimbursed = rs.getInt("reimbursed") == 1;
                    String note = rs.getString("note");
                    list.add(new Expense(id, amount, date, cid, cname, project, payment, reimbursed, note));
                }
            }
        } catch (SQLException e) {
            e.printStackTrace();
    ...
    public static void insertExpense(Expense e) {
        String sql = """
                INSERT INTO expenses (amount, date, category_id, project, payment_method, reimbursed, note)
                VALUES (?, ?, ?, ?, ?, ?, ?)
                """;
        try (Connection conn = Database.getConnection();
                PreparedStatement ps = conn.prepareStatement(sql)) {
            ...
            ps.setString(4, e.getProject());
            ps.setString(5, e.getPaymentMethod());
            ps.setInt(6, e.isReimbursed() ? 1 : 0);
    ...
    public static void updateExpense(Expense e) {
        String sql = """
                UPDATE expenses
                SET amount = ?, date = ?, category_id = ?, project = ?, payment_method = ?, reimbursed = ?, note = ?
                WHERE id = ?
                """;
        try (Connection conn = Database.getConnection();
                PreparedStatement ps = conn.prepareStatement(sql)) {
    ...
    public static Map<String, Double> getMonthlyTotalsByProject(int year, int month) {
        Map<String, Double> map = new LinkedHashMap<>();
        String sql = """
                SELECT IFNULL(project, 'No Project') AS proj, SUM(amount) AS total
                FROM expenses
                WHERE strftime('%Y', date) = ?
                  AND strftime('%m', date) = ?
                GROUP BY proj
        ...
        try (Connection conn = Database.getConnection();
            ...
            try (ResultSet rs = ps.executeQuery()) {
                while (rs.next()) {
                    map.put(rs.getString("proj"), rs.getDouble("total"));
                }
            }
        } catch (SQLException e) {
            e.printStackTrace();
