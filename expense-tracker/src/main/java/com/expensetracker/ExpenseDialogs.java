...
public class ExpenseDialogs {
    ...
    public static Optional<Expense> showExpenseDialog(Stage owner, Expense existing) {
        ...
        projectField.setPromptText("Client or Project Name");

        ComboBox<String> paymentMethodBox = new ComboBox<>();
        paymentMethodBox.getItems().addAll("Cash", "Personal Card", "Corporate Card", "Bank Transfer");
        paymentMethodBox.setValue("Cash");

        CheckBox reimbursedCheck = new CheckBox("Reimbursed");
        ...
        if (existing != null) {
            ...
            projectField.setText(existing.getProject());
            paymentMethodBox.setValue(existing.getPaymentMethod());
            reimbursedCheck.setSelected(existing.isReimbursed());
        ...
        grid.add(new Label("Project:"), 0, 3);
        grid.add(projectField, 1, 3);

        grid.add(new Label("Payment:"), 0, 4);
        grid.add(paymentMethodBox, 1, 4);

        grid.add(reimbursedCheck, 1, 5);
        ...
        grid.add(btnPane, 1, 7);

        Scene scene = new Scene(grid, 420, 380); // Increased height for more fields
        ...
        okBtn.setOnAction(e -> {
            try {
                ...
                String catName = categoryBox.getEditor().getText();
                String amountStr = amountField.getText();
                String project = projectField.getText();
                String payment = paymentMethodBox.getValue();
                boolean reimbursed = reimbursedCheck.isSelected();
                ...
                if (existing == null) {
                    // New expense
                    resultHolder[0] = new Expense(amount, date, catId, catName.trim(), project, payment, reimbursed,
                            note);
                } else {
                    // Update existing
                    ...
                    existing.setPaymentMethod(payment);
                    existing.setReimbursed(reimbursed);
